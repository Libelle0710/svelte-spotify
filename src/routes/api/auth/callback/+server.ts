...
export const GET: RequestHandler = async ({url, cookies}) => {
    ...
    const response = await fetch('https://accounts.spotify.com/api/token', {
		method: 'POST', //make a POST request to the Spotify Accounts service's token endpoint
		headers: {
			'Content-Type': 'application/x-www-form-urlencoded', //standard content type for form data
			Authorization: `Basic ${Buffer.from(
				`${SPOTIFY_APP_CLIENT_ID}:${SPOTIFY_APP_CLIENT_SECRET}`
			).toString('base64')}` //client credentials (client ID and client secret) 
            //in the form of a Basic Authentication header. The client ID and secret are Base64-encoded
		},
		body: new URLSearchParams({
			code: code || '', //he authorization code received from the previous step
